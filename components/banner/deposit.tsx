...
export default function Component(props: DepositBannerProps) {
  ...
  return (
    <div className="flex-initial flex gap-x-4 flex-wrap gap-y-4">
      ...
      <Item
        title="보관 수량"
        value={`${
          _.isFinite(props.spec.quantity)
            ? format("packed")(getQuantity(props.spec.quantity ?? 0))
            : ""
        } ${
          props.spec.packaging.type !== "ROLL"
            ? ` (${
                _.isFinite(props.spec.quantity)
                  ? format("unpacked")(getQuantity(props.spec.quantity ?? 0))
                  : ""
              })`
            : ""
        }`}
      ...
      <Item
        ...
        value={
          _.isFinite(props.spec.quantity)
            ? format("weight")(getQuantity(props.spec.quantity ?? 0))
            : ""
        }
      />
    </div>
