...
export default function Component(props: Props) {
  ...
  const apiByBankAccount =
    ApiHook.Partner.ByBankAccount.useByBankAccountCreate();
      ...
      switch (method) {
        case "ACCOUNT_TRANSFER":
          await apiByBankAccount.mutateAsync({
            data: values as Api.ByBankAccountCreateRequest,
          });
          break;
        case "CARD_PAYMENT":
          const cardReq = values as Api.ByCardCreateRequest;
          if (
            values.amount < (values as Api.ByCardCreateRequest).chargeAmount
          ) {
        ...
        case "OFFSET":
          await apiByOffset.mutateAsync({
            data: values as Api.ByOffsetCreateRequest,
          });
          break;
        case "CASH":
          await apiByCash.mutateAsync({
            data: values as Api.ByCashCreateRequest,
          });
          break;
        case "ETC":
          await apiByEtc.mutateAsync({
            data: values as Api.ByEtcCreateRequest,
          });
          break;
      }
  ...
  return (
    <Popup.Template.Property
      title={`${props.accountedType === "PAID" ? "지급" : "수금"} 등록`}
      width="800px"
      {...props}
    >
      {contextHolder}
